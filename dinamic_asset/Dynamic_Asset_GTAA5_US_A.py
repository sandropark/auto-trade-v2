...
BOT_NAME = common.get_now_dist() + "_MyGTAA5BotUS"
...
# 0 16 23-28 * 1-5 python3 /var/autobot/Dynamic_Asset_GTAA5_US.py
...
for stock_info in my_portfolio_list:
    ...
    #현재가!
    current_price = KisUS.get_current_price(stock_code)
...
my_stock_list = KisUS.get_my_stock_list("KRW")
...
for stock_info in my_portfolio_list:
    ...
    if stock_amt > 0:
        ...
        if IsMaUp == True:
            ...
            if stock_now_rate != stock_target_rate:
                ...
                gap_amt = gap_money / (current_price * float(KisUS.get_exrt()))
    ...
    # 잔고에 없는 경우
        ...
        if IsMaUp == True:
            ...
            ########################################################
            # 매수할 수량을 계산한다!
            buy_amt = int(buy_money / (current_price * float(KisUS.get_exrt())))
...
if is_rebalance_go == True and is_market_open == True:
        ...
        if rebalance_amt < 0:
            ...
            pprint.pprint(
                KisUS.make_sell_limit_order(
                    stock_code, abs(rebalance_amt), current_price
                )
            )
    ...
    for stock_info in my_portfolio_list:
        ...
        if rebalance_amt > 0:
            ...
            current_price *= 1.1
            pprint.pprint(
                KisUS.make_buy_limit_order(stock_code, rebalance_amt, current_price)
            )
